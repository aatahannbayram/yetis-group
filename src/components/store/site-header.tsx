...
export async function SiteHeader({
  ...
  const inner = (
    ...
    >
      <Link href="/" className="flex min-w-0 shrink items-center">
        {isOverlay ? (
          <>
            <Logo variant="dark" size="md" className="brightness-0 invert md:hidden" />
            <Logo variant="dark" size="xl" className="hidden brightness-0 invert md:block" />
          </>
        ) : (
          <>
  ...
  return (
    <header
      className={cn("z-40 w-full", isOverlay ? "absolute inset-x-0 top-0" : "sticky top-0")}
    >
      {isOverlay ? inner : <StickyHeaderShell>{inner}</StickyHeaderShell>}
    </header>
  );
}
